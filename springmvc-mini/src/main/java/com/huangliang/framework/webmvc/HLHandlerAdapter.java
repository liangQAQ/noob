...
public class HLHandlerAdapter {

    public HLModelAndView handle(HttpServletRequest req, HttpServletResponse resp, Object handler)
            throws Exception{
        HLHandlerMapping handlerMapping = (HLHandlerMapping)handler;
        ...
        for (int i = 0; i < pa.length ; i ++) {
            for (Annotation a : pa[i]) {
                if(a instanceof HLRequestParam){
                    String paramName = ((HLRequestParam) a).value();
